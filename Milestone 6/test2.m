...
while true
    ...
    for gen = 1:TLBO.MaxIterations
        ...
        bestcostarr = [bestcostarr; costs(best_idx)];

        % Update Fitness Plots
        set(plot_per_run, 'YData', bestcostarr, 'XData', 1:length(bestcostarr));
        all_best_cost_log = [all_best_cost_log; best_cost_log(gen)];
    ...
    % Check Completion
    % Calculate distance to designated targets
    % Correcting the distances calculation
