...
% PSO Parameters
PSO.SwarmSize = 20;
PSO.MaxIterations = 60; % Max number of iterations per run
PSO.c1 = 1.5;
PSO.c2 = 0.6;
...
all_best_cost_log = []; % Store fitness values across runs
...
% Add variables for dynamic inertia weight
PSO.weight_initial = 0.99;  % Starting inertia weight
PSO.weight_final = 0.7;    % Final inertia weight

% Initialize overall fitness tracking variables
all_best_cost_log = []; % Store fitness values across all runs
bestcostarr = [];       % Store fitness values for each PSO run

% Main PSO loop
while true
    best_cost_log = zeros(PSO.MaxIterations, 1); % Track fitness per iteration in the current run
    penalty_value = 1e3; % Penalty for constraint violation

    for iter = 1:PSO.MaxIterations
        % Adjust inertia weight dynamically
        PSO.weight = PSO.weight_initial - ...
            ((PSO.weight_initial - PSO.weight_final) * (iter / PSO.MaxIterations));
        ...
        % Introduce swarm diversity every 10 iterations
        if mod(iter, 10) == 0
            num_random_particles = floor(PSO.SwarmSize * 0.1); % Replace 10% of swarm
            for i = 1:num_random_particles
                bird = zeros(4, num_UAVs);
                for j = 1:num_UAVs
                    bird(:, j) = [rand * 2 - 1; rand * 2 - 1; p_UAV_init(:, j)];
                end
                swarm{randi([1 PSO.SwarmSize])} = bird; % Randomly replace a particle
            end
        end
    ...
    % Reinitialize swarm for the next PSO run
    for i = 1:PSO.SwarmSize
        bird = zeros(4, num_UAVs);
        for j = 1:num_UAVs
            bird(:, j) = [rand * 2 - 1; rand * 2 - 1; p_UAV_init(:, j)];
    ...
    % Reset personal bests but keep global best
    %personal_best_costs = inf(PSO.SwarmSize, 1);
    %personal_best_positions = cell(PSO.SwarmSize, 1);

    % Update UAV positions and paths
    for j = 1:num_UAVs
        direction = global_best_position(3:4, j)' - uav_positions(j, :);
        step = direction / max(norm(direction), 1e-3); % Normalize direction
        uav_positions(j, :) = uav_positions(j, :) + step; % Move by one step
        all_paths{j} = [all_paths{j}; uav_positions(j, :)]; % Append new position
    end

    % Update UAV positions and paths in the subplot
    subplot(1, 3, 1); % Ensure subplot 1 is active
    hold on;
    for j = 1:num_UAVs
        % Update the UAV path plot
        plot(all_paths{j}(:, 1), all_paths{j}(:, 2), '-', 'LineWidth', 1.5, ...
            'DisplayName', ['UAV ' num2str(j)]);
    end

    % Update UAV position markers
...
num_UAVs = size(x, 2);
d_safe = 1.0;  % Safe distance between UAVs
restricted_area_center = [125, 125];
restricted_area_radius = 15;
...
for i = 1:num_UAVs
    distance_to_restricted = norm(x(3:4, i)' - restricted_area_center) - (restricted_area_radius + 15);
